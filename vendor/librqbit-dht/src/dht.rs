...
impl DhtWorker {
    ...
    async fn framer(
        ...
        let reader = async {
            // WinT patch. Two changes, both about one bad datagram not being
            // able to kill the DHT for the rest of the process's life.
            //
            // The buffer was 16 KiB. On Windows a datagram larger than the
            // buffer handed to recvfrom does not truncate as it does on Unix:
            // the call fails with WSAEMSGSIZE (10040) and the packet is
            // dropped. Sized to the largest a UDP payload can be, so a legal
            // packet is never the cause.
            let mut buf = vec![0u8; 65536];
            // A receive error used to propagate, ending the framer and with it
            // the whole DHT — permanently, while the session stayed up and
            // waited for a DHT that would never answer again. Transient errors
            // are normal on a UDP socket (an ICMP port-unreachable from a dead
            // peer surfaces as one on Windows), so they are now treated the
            // same way a malformed message already was: logged, and skipped.
            //
            // The consecutive count still bounds it, so a socket that has
            // genuinely broken ends the framer rather than spinning.
            const MAX_CONSECUTIVE_RECV_ERRORS: usize = 128;
