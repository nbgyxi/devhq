...
public static class WinTAudio {
 ...
 public static string Test(string id,string flow) { var e=(IEnum)new Enumerator();IDevice d;int rc=e.GetDevice(id,out d);if(rc!=0)Marshal.ThrowExceptionForHR(rc);if(flow=="recording"){int muted=0;rc=Volume(d).GetMute(out muted);if(rc!=0)Marshal.ThrowExceptionForHR(rc);if(muted!=0)throw new Exception("This microphone is muted. Unmute it before checking input.");var iid=new Guid("C02216F6-8C67-4B5B-9D00-D008E73E0064");object o;rc=d.Activate(ref iid,23,IntPtr.Zero,out o);if(rc!=0)Marshal.ThrowExceptionForHR(rc);var meter=(IMeter)o;float peak=0;for(int i=0;i<40;i++){float value;rc=meter.GetPeakValue(out value);if(rc!=0)Marshal.ThrowExceptionForHR(rc);peak=Math.Max(peak,value);System.Threading.Thread.Sleep(50);}return "MIC_PEAK:"+Math.Ceiling(peak*100);}Set(id);const int rate=44100,ms=500,samples=rate*ms/1000;var stream=new System.IO.MemoryStream();var w=new System.IO.BinaryWriter(stream);w.Write(System.Text.Encoding.ASCII.GetBytes("RIFF"));w.Write(36+samples*2);w.Write(System.Text.Encoding.ASCII.GetBytes("WAVEfmt "));w.Write(16);w.Write((short)1);w.Write((short)1);w.Write(rate);w.Write(rate*2);w.Write((short)2);w.Write((short)16);w.Write(System.Text.Encoding.ASCII.GetBytes("data"));w.Write(samples*2);for(int i=0;i<samples;i++)w.Write((short)(Math.Sin(2*Math.PI*660*i/rate)*9000));stream.Position=0;new System.Media.SoundPlayer(stream).PlaySync();return "Test sound played through the selected device"; }
}
'@;"#;
