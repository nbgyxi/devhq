(() => {
  ...
  function renderSidebar(tool) {
    ...
    if (!barKnown) loadBar();
    const scrollTop = host.querySelector(".win-tool-body")?.scrollTop || 0;
    const edges = [["left", "Left edge"], ["right", "Right edge"]];
    host.innerHTML = header(tool, `<div class="dock-page">
      <section class="dock-card">
        <div class="dock-card-head"><h3>Docking</h3><div class="win-status" data-win-status data-tone="${dock.docked ? "ok" : ""}">${dock.docked ? `Docked to the ${esc(dock.edge)} edge. The taskbar is ${dock.taskbarWindowsHidden ? "hidden completely" : dock.taskbarAutoHidden ? "auto-hidden" : "left as you had it"}.` : "Not docked. The desktop work area is untouched."}</div></div>
        ...
        <label class="dock-slot dock-slot-sub"><input type="checkbox" data-dock-hide-taskbar-completely${bar.hideTaskbarCompletely === true ? " checked" : ""}${bar.hideTaskbar === false ? " disabled" : ""}>${icon("visibility_off")}<span>Hide it completely, so notifications cannot slide it back in</span></label>
        ...
        <p>Auto-hiding only slides the taskbar off the edge: Explorer brings it straight back over the rail whenever an app asks for attention with a notification. Hiding it completely takes its windows away instead, so nothing reveals it — not a notification, not the pointer, not the Windows key. If WinT ever stops responding while that is on, press Ctrl+Shift+Esc, find Windows Explorer and restart it; a sign-out or a reboot brings the taskbar back too.</p>
      </section>
      <section class="dock-card dock-contents-card">
        <div class="dock-card-title">${icon("view_sidebar")}<span><h3>Bar contents</h3><p>Choose which controls earn a place on the rail.</p></span></div>
        ...
        <label class="dock-slot"><input type="checkbox" data-dock-clock-seconds${bar.clockSeconds === true ? " checked" : ""}>${icon("timer")}<span>Show seconds on the clock</span></label>
      </section>
      <section class="dock-card dock-tools-card">
        <div class="dock-card-title">${icon("build")}<span><h3>Tool shortcuts</h3><p>Each opens in its own window from a section above the bar controls.</p></span></div>
        ${dockToolsHtml()}
      </section>
      <section class="dock-card">
        <div class="dock-card-title">${icon("music_note")}<span><h3>Music player</h3><p>Control when the compact player appears below your tool shortcuts.</p></span></div>
        <div class="dock-choice-grid">
          ${[["always","Always","Keep the latest player ready; its app tag can start the app."],["playing","While playing","Show it only while Windows reports active playback."],["never","Never","Leave the music player off the sidebar."]].map(([id,title,note]) => `<label class="dock-choice${(bar.mediaMode || "playing") === id ? " on" : ""}"><input type="radio" name="dock-media" value="${id}" data-dock-media${(bar.mediaMode || "playing") === id ? " checked" : ""}><span>${icon(id === "always" ? "keep" : id === "playing" ? "graphic_eq" : "visibility_off")}<strong>${title}</strong><small>${note}</small></span></label>`).join("")}
        </div>
      </section>
      <section class="dock-card">
        <div class="dock-card-title">${icon("palette")}<span><h3>Appearance</h3><p>Changes show on the sidebar straight away.</p></span></div>
        <div class="dock-choice-grid dock-size-choices">
          ${[["small",14,14,14,"Compact","14 px icons · 14 px text · 14 px tray"],["large",20,16,20,"Large","20 px icons · 16 px text · 20 px tray"]].map(([id,iconSize,textSize,trayIconSize,title,note]) => `<label class="dock-choice${bar.iconSize === iconSize && bar.textSize === textSize && (bar.trayIconSize || 14) === trayIconSize ? " on" : ""}"><input type="radio" name="dock-size" value="${id}" data-dock-preset data-icon-size="${iconSize}" data-text-size="${textSize}" data-tray-icon-size="${trayIconSize}"${bar.iconSize === iconSize && bar.textSize === textSize && (bar.trayIconSize || 14) === trayIconSize ? " checked" : ""}><span>${icon(id === "small" ? "density_small" : "density_large")}<strong>${title}</strong><small>${note}</small></span></label>`).join("")}
        </div>
        <div class="dock-size-sliders">
          ...
          <label class="dock-row"><span>Tray icons</span><input type="range" min="10" max="24" step="1" value="${bar.trayIconSize || 14}" data-dock-size="trayIconSize"><code data-dock-size-out="trayIconSize">${bar.trayIconSize || 14} px</code></label>
        </div>
      </section>
    </div>`);
    // Saving a checkbox or tool rebuilds this isolated page. Keep the user's
    // place instead of snapping back to the top after every live change.
    requestAnimationFrame(() => {
      const scroller = host.querySelector(".win-tool-body");
  ...
  let dockToolQuery = "";
  let dockToolFilter = "common";
  const DOCK_COMMON_TOOLS = ["git", "explorer", "github", "ports", "clipboard", "keep-awake", "focus-mode", "security-audit", "disk-space", "dns", "startup", "time-tracker"];
  ...
  function renderSecurityAudit(tool) {
    ...
    script.onload = mount;
    script.onerror = () => { node.innerHTML = '<div class="win-empty">PC Detective could not load.</div>'; };
  ...
  // Its own file: the tool is two long lists with their own refresh, and the
  // page is only ever open when someone is looking at it.
  function renderStartupTray(tool) {
    host.innerHTML = header(tool, '<div data-startup-host></div>');
    const node = host.querySelector("[data-startup-host]");
    const mount = () => { if (node.isConnected) window.wintStartupTray.mount(node); };
    if (window.wintStartupTray) return mount();
    const script = document.createElement("script");
    script.src = "startup-tray.js";
    script.onload = mount;
    script.onerror = () => { node.innerHTML = '<div class="win-empty">Startup and tray could not load.</div>'; };
    document.head.appendChild(script);
  }
  ...
  */
  ...
  async function loadAudioDevices(){
    try{const rows=await work("audio-devices","Reading audio devices",invoke("audio_devices"));const groups=[['playback','Playback','volume_up'],['recording','Recording','mic']];
      host.querySelector("[data-audio-groups]").innerHTML=groups.map(([flow,label,glyph])=>`<section class="audio-group"><header>${icon(glyph)}<strong>${label}</strong><small>Console · Multimedia · Communications</small></header>${rows.filter((r)=>r.flow===flow).map((r)=>`<div class="audio-device-row${r.isDefault?' on':''}${r.muted?' muted':''}"><button class="audio-device" data-audio-device="${esc(r.id)}"><span>${icon(r.isDefault?'check_circle':'radio_button_unchecked')}<strong>${esc(r.name)}</strong></span><small>${r.isDefault?'Default for all roles':'Set as default for all roles'}</small></button><div class="audio-device-controls"><label class="audio-volume"><span>${icon(r.muted?'volume_off':'volume_up')}<output>${r.volume}%</output></span><input type="range" min="0" max="100" value="${r.volume}" data-audio-volume="${esc(r.id)}" aria-label="Volume for ${esc(r.name)}" ${r.muted?'disabled':''}></label><button type="button" class="audio-mute-status ${r.muted?'is-muted':'is-unmuted'}" data-audio-muted="${esc(r.id)}" data-muted="${r.muted}" title="${r.muted?'Unmute':'Mute'} ${esc(r.name)}" aria-label="${r.muted?'Unmute':'Mute'} ${esc(r.name)}">${icon(r.muted?'volume_off':'volume_up')}<span><small>Mute status</small><strong>${r.muted?'Muted':'Not muted'}</strong></span></button><button class="btn audio-mute-action" data-audio-muted="${esc(r.id)}" data-muted="${r.muted}" aria-label="${r.muted?'Unmute':'Mute'} ${esc(r.name)}">${icon(r.muted?'volume_up':'volume_off')} ${r.muted?'Unmute':'Mute'}</button><button class="btn" data-audio-test="${esc(r.id)}" data-audio-flow="${flow}"${flow==='recording'&&r.muted?' disabled title="Unmute this microphone before checking input"':''}>${icon(flow==='playback'?'play_arrow':'mic')} ${flow==='playback'?'Play test':'Check input'}</button></div>${flow==='recording'?`<div class="audio-input-result" data-audio-test-result aria-live="polite">${icon(r.muted?'mic_off':'graphic_eq')}<span>${r.muted?'This microphone is muted. Unmute it before checking input.':'Speak into the microphone, then choose Check input.'}</span></div>`:''}</div>`).join('')||'<div class="win-empty">No active devices.</div>'}</section>`).join('');status(`${rows.length} active audio device${rows.length===1?'':'s'}`,"ok");
    }catch(error){status(String(error),"bad");}}
  ...
  async function testAudioDevice(id,flow,button){const playback=flow==='playback';const resultBox=button.closest('.audio-device-row')?.querySelector('[data-audio-test-result]');button.disabled=true;button.classList.add('spinning');button.setAttribute('aria-busy','true');button.innerHTML=`${icon('progress_activity')} ${playback?'Playing…':'Listening…'}`;if(resultBox){resultBox.className='audio-input-result listening';resultBox.innerHTML=`${icon('graphic_eq')}<span>Listening for 2 seconds… speak now.</span>`;}status(playback?'Playing a test sound…':'Listening to the selected microphone for 2 seconds…');try{const result=await work("audio-test",playback?'Playing test sound':'Checking microphone input',invoke("audio_test",{id,flow}));if(!result.ok){if(resultBox){resultBox.className='audio-input-result bad';resultBox.innerHTML=`${icon('error')}<span>${esc(result.error||'Windows could not read this microphone.')}</span>`;}return status(result.error||"Could not test the device.","bad");}if(playback){await loadAudioDevices();return status(result.output||"Test sound played.","ok");}const peak=Number((result.output||'').match(/MIC_PEAK:(\d+)/)?.[1]);const detected=Number.isFinite(peak)&&peak>0;if(resultBox){resultBox.className=`audio-input-result ${detected?'ok':'warn'}`;resultBox.innerHTML=`${icon(detected?'check_circle':'mic_off')}<span><strong>${detected?`Input detected · Peak ${peak}%`:'No input detected'}</strong><small>${detected?'The selected microphone is working.':'Check mute, the physical mic switch, and Windows microphone privacy settings.'}</small></span>${detected?`<meter min="0" max="100" value="${peak}" aria-label="Peak input ${peak}%"></meter>`:''}`;}status(detected?`Microphone input detected; peak ${peak}%.`:'No microphone input was detected.',detected?'ok':'warn');}catch(error){if(resultBox){resultBox.className='audio-input-result bad';resultBox.innerHTML=`${icon('error')}<span>${esc(String(error))}</span>`;}status(String(error),'bad');}finally{if(button.isConnected){button.disabled=false;button.classList.remove('spinning');button.removeAttribute('aria-busy');button.innerHTML=`${icon(playback?'play_arrow':'mic')} ${playback?'Play test':'Check input'}`;}}}
  ...
  function click(event) {
    ...
    const dockToolFilterButton=event.target.closest("[data-dock-tool-filter]");if(dockToolFilterButton){dockToolFilter=dockToolFilterButton.dataset.dockToolFilter;return renderSidebar(catalog.find((x)=>x.id==="sidebar"));}
    if(event.target.closest("[data-dock-tools-clear]")){bar={...bar,tools:[]};saveBar();return renderSidebar(catalog.find((x)=>x.id==="sidebar"));}
    if(event.target.closest("[data-dock-toggle]"))return dockCall(dock.docked?"sidebar_close":"sidebar_open",dock.docked?undefined:{edge:dock.edge,width:dock.width,hideTaskbar:bar.hideTaskbar!==false,hideCompletely:bar.hideTaskbarCompletely===true});
